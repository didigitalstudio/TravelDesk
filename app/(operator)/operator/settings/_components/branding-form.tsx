...
export function OperatorBrandingForm({
  operatorId,
  initialName,
  initialContactEmail,
  initialBrandColor,
  ...
  const [state, action, pending] = useActionState(updateOperatorProfile, initialState);
  ...
  function onPickFile(file: File) {
    ...
    startUpload(async () => {
      ...
      const res = await setOperatorLogoUrl(pub.publicUrl);
      if (!res.ok) {
  ...
  function onRemoveLogo() {
    ...
    startUpload(async () => {
      ...
      const path = m?.[1];
      if (path) {
        await deleteOperatorLogo(decodeURIComponent(path));
      } else {
        await setOperatorLogoUrl(null);
      }
      setLogoUrl(null);
    });
  }
  ...
  return (
    <section className="surface p-6">
      <header className="mb-4">
        <h2 className="text-base font-semibold text-zinc-100">Datos del operador</h2>
        <p className="mt-1 text-xs text-zinc-500">
          Cómo te ven las agencias y los clientes finales.
        </p>
      </header>
      ...
      <form action={action} className="space-y-4">
        <Field label="Nombre" htmlFor="op-name">
          <input
            id="op-name"
            name="name"
        ...
        <Field label="Email de contacto" htmlFor="op-contact">
          <input
            id="op-contact"
            name="contact_email"
            type="email"
        ...
        <Field label="Color de marca" htmlFor="op-color">
